...
CREATE FUNCTION ENERGY_PRODUCTION_LAST_MONTH ( @POWER_PLANT_ID int, @month int )
RETURNS float
BEGIN
	DECLARE @total_energy_production float
	SET @total_energy_production = (SELECT SUM(LOG_POWER_OUTPUT) FROM POWER_FLUX_LOGS (NOLOCK) WHERE LOG_SOURCE_ID = @POWER_PLANT_ID AND MONTH(LOG_TIMESTAMP) = @month)
	RETURN @total_energy_production
END
GO

-- FUNÇÃO PARA CALCULAR A ENERGIA CONSUMIDA NUM DETERMINADO MÊS
CREATE FUNCTION ENERGY_CONSUMPTION_LAST_MONTH ( @POWER_PLANT_ID int, @month int )
RETURNS float
BEGIN
	DECLARE @total_energy_consumption float
	SET @total_energy_consumption = (SELECT SUM(LOG_POWER_CONSUMPTION) FROM POWER_FLUX_LOGS WHERE LOG_SOURCE_ID = @POWER_PLANT_ID AND MONTH(LOG_TIMESTAMP) = @month)
	RETURN @total_energy_consumption
END
GO

-- FUNÇÃO PARA CALCULAR A ENERGIA ARMAZENADA NUM DETERMINADO MÊS
CREATE FUNCTION ENERGY_STORAGE_LAST_MONTH ( @POWER_PLANT_ID int, @month int )
RETURNS float
BEGIN
	DECLARE @total_energy_storage float
	SET @total_energy_storage = (SELECT SUM(LOG_POWER_STORAGE) FROM POWER_FLUX_LOGS WHERE LOG_SOURCE_ID = @POWER_PLANT_ID AND MONTH(LOG_TIMESTAMP) = @month)
